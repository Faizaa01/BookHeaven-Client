...
const EditBook = () => {
  ...
  useEffect(() => {
    ...
    authApiClient.get(`/books/${bookId}/`).then(res => {
      const book = res.data;
      console.log(book)
      setValue('title', book.title);
      setValue('author', book.author.id);
      setValue('category', book.category.id);
      setValue('isbn', book.isbn);
      setValue('availability', book.availability_status ? 'available' : 'unavailable');
      setPreviewImages(book.images.map(img => img.image)); // existing images
    });
  }, [bookId, setValue]);
  ...
  const handleBookUpdate = async (data) => {
    try {
      ...
      await authApiClient.put(`/books/${bookId}/`, payload);
  ...
  return (
    <section className='bg-rose-100 py-20' style={{backgroundImage: `url(${bg})`, backgroundSize: 'cover'}}>
      <div className="max-w-xl mx-auto mt-12 p-8 shadow-md rounded-lg bg-white/30 backdrop-blur-xl">
        <h2 className="text-3xl font-semibold mb-6 text-gray-800 text-center">Edit Book</h2>

        <form onSubmit={handleSubmit(handleBookUpdate)} className="space-y-6">
          <div>
            ...
            />
            {errors.title && <p className="text-red-600 text-xs mt-1">This field is required</p>}
          </div>
          ...
          <div>
            ...
            {errors.author && <p className="text-red-600 text-xs mt-1">This field is required</p>}
          </div>
          ...
          <div>
            ...
            {errors.category && <p className="text-red-600 text-xs mt-1">This field is required</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="ISBN"
            />
            {errors.isbn && <p className="text-red-600 text-xs mt-1">This field is required</p>}
          </div>
        ...
        <div className="mt-6">
          <h3 className="text-xl font-semibold mb-4 text-gray-800">Update Images</h3>
          <input
            type="file"
            multiple
            accept="image/*"
          ...
          {previewImages.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-4">
              {previewImages.map((src, idx) => (
                <img key={idx} src={src} alt={`Preview ${idx+1}`} className="w-20 h-20 rounded-lg object-cover border border-gray-200"/>
              ))}
            </div>
          )}
          <button
